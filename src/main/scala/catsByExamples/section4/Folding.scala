package catsByExamples.section4
...
object Folding {
  ...
  val anotherSum =
    Foldable[List].combineAll(List(1, 2, 3)) // implicit Monoid[Int]
  import cats.instances.string._
  val mappedConcat = Foldable[List].foldMap(List(1, 2, 3))(
    _.toString
  ) // implicit Monoid[String] => "123"
  ...
  val mappedConcat2 =
    List(1, 2, 3).foldMap(_.toString) // requires Foldable[List], Monoid[String]
